...
seed=100
iters=1
old_id=${model}_${dataset}_dropout${dropout}_bs${batch_size}_iter${iters}_gpu${gpu_num}_rnn${rnn_size}_numlayers${num_layers}_seed${seed}_${cnn_model}
id=tmp
if [ ! -d "log/$id"  ]; then
    mkdir -p log/$id
fi
...
CUDA_VISIBLE_DEVICES=1 python \
    ...
    --input_encoding_size 1024 --topic_num 200 \
    --use_img 0 --use_topic 1 --use_fc 1 \
    --input_json data/$dataset/${dataset}_meta.json \
    --img_fold data/$dataset/images --img_size 320 --img_csize 299 \
    --input_fc_dir data/$dataset/feat299_${cnn_model}_fc_baseline\
    --input_label_h5 data/$dataset/${dataset}_label.h5 \
    --input_topic_h5 data/$dataset/${dataset}_gt_topics.h5 \
