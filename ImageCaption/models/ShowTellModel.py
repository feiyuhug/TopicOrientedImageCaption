...
class ShowTellModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.hard_thresh = opt.hard_thresh
        ...
        self.core = RNN('LSTM', self.input_encoding_size, self.rnn_size, num_layers=self.num_layers, bias=False, dropout=self.drop_prob_lm, hard_thresh=self.hard_thresh, norm_thresh=0.01)
    ...
    def forward(self, fc_feats, att_feats, topics, seq):
        ...
        outputs = []
        gate_norm_sum, gate_norm_c_sum, hard_norm_sum, hard_norm_c_sum = 0, 0, 0, 0
        for i in range(seq.size(1)):
            if i == 0:
                ...
                xt = self.embed(it)
            xt = self.dropout(xt)
            output, state, gate_norm, hard_norm = self.core(xt.unsqueeze(0), state)
        ...
        for i in range(seq.size(1)  - len(outputs)):
            outputs.append(Variable(torch.zeros(outputs[-1].data.size()).cuda()))
        #return torch.cat([_.unsqueeze(1) for _ in outputs[1:]], 1).contiguous(), (gate_norm_sum, gate_norm_c_sum), (hard_norm_sum, hard_norm_c_sum)
