...
class Datasource():
    ...
    def next(self):
        ...
        cap_tps = self.data['cap_tps'][numpy.array(caption_ids)]
        im_tps = self.data['im_tps'][numpy.array(image_ids)]
        
        return image_ids, x, x_mask, im, cap_tps, im_tps

    def all(self):
        return self.data['caps'], self.data['ims'], self.data['cap_tps'], self.data['im_tps']
