...
def encode_topics() :
  im_top10topics = []
  cap_top10topics = []
  for split in ['val2014'] :
    im_tps = np.load('../data/annotations/im_topics_detect_%s_t200_lmdb1bak_resnet.npy'%(split))
    #cap_tps = np.load('../data/annotations/doc-topic_%s_line_t200.npy'%(split))
    im_top10topics.append((0 - im_tps).argsort(axis=1)[:,0:10] + 1)
    #cap_top10topics.append((0 - cap_tps).argsort(axis=1)[:,0:10] + 1)
  im_top10topics = np.concatenate(im_top10topics, axis=0)
  #cap_top10topics = np.concatenate(cap_top10topics, axis=0)
  #return im_top10topics.astype('uint32'), cap_top10topics.astype('uint32')
  return im_top10topics.astype('uint32')
...
def main2(params) :
  #im_top10topics, cap_top10topics = encode_topics()
  im_top10topics = encode_topics()
  f = h5py.File(params['output_h5'], "w")
  f.create_dataset("im_top10topics", dtype='uint32', data=im_top10topics)
  #f.create_dataset("cap_top10topics", dtype='uint32', data=cap_top10topics)
  f.close()
