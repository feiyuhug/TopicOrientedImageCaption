...
function net_utils.build_cnn(cnn, opt)
  local layer_num = utils.getopt(opt, 'layer_num', 45)
...
function net_utils.build_cnn_resnet(cnn, opt)
  ...
  resnet_bn_mean_var_104 = npy4th.loadnpz('resnet_bn_mean_var_104.npz')
  local raw_cnn_module_list = net_utils.list_nngraph_modules(cnn)
  local bn_item_ind = 1
  for k,m in ipairs(raw_cnn_module_list) do
    if m.running_mean then
      print(bn_item_ind, m)
      --print(resnet_bn_mean_var_104[tostring(bn_item_ind)]:mean(), resnet_bn_mean_var_104[tostring(bn_item_ind + 1)]:mean())
      --print(m.running_mean:mean(), m.running_var:mean())
      m.running_mean = resnet_bn_mean_var_104[tostring(bn_item_ind)]
      m.running_var = resnet_bn_mean_var_104[tostring(bn_item_ind + 1)]
      bn_item_ind = bn_item_ind + 2
    end
  end
  print('total bn layer number :', (bn_item_ind - 1)/2)
  --local cnn_part = nn.Sequential()
  --cnn_part:add(cnn:cuda())
  --cnn_part:add(nn.Normalize(2))
  --cnn_part:add(backend.ReLU(true))
  local emb_input = -nn.Identity()
  local emb_output = emb_input - cnn - nn.Normalize(2) - backend.ReLU(true)
  cnn_part = nn.gModule({emb_input}, {emb_output})
  return cnn_part
end

function net_utils.build_fcn(fcn, opt)
  local layer_num = utils.getopt(opt, 'layer_num', 1)
  local backend = utils.getopt(opt, 'backend', 'cudnn')
  ...
  -- copy over the first layer_num layers of the CNN
  local fcn_part = nn.Sequential()
  for i = 2, layer_num do
    local layer = fcn:get(i)
    fcn_part:add(layer)
  end
  fcn_part:add(nn.Normalize(2))
  fcn_part:add(backend.ReLU(true))

  return fcn_part
end

function net_utils.build_fcn_fromt7()
  local backend = utils.getopt(opt, 'backend', 'cudnn')
  ...
  -- copy over the first layer_num layers of the CNN
  emb_params = npy4th.loadnpz('../order-embedding/snapshots/sentence-image-finetuned-t200-lmdb1bak-topic-insert_0.2_resnet_order_1.npz')
  local emb_layer = nn.Linear(200, 1024)
  emb_layer.weight = emb_params['ff_topic_W']:transpose(1,2)
  emb_layer.bias = emb_params['ff_topic_b']
  local fcn_part = nn.Sequential()
  fcn_part:add(emb_layer)
  fcn_part:add(nn.Normalize(2))
  fcn_part:add(backend.ReLU(true))
...
function net_utils.prepro(imgs, data_augment, on_gpu, net_type)
  ...
  -- subtract vgg mean
    ...
    imgs:add(-1, net_utils.resnet_mean:expandAs(imgs))
  end
  return imgs
end
...
function net_utils.forth_unsanitize_gradients(net)
  local moduleList = net_utils.listModules(net)
  --print(moduleList)
  for k,m in ipairs(moduleList) do
    if m.weight then
